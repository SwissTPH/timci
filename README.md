...
`INSTITUTION` is an environment variable that sets the name of your institution.

`PII_PW` is an environment variable that sets the password of the encrypted archive that is generated by the R pipeline to store confidential information (and in particular, participant personally identifiable information).
