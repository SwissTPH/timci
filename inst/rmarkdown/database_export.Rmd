...
hospit_data <- NULL
if (crf_hospit_fid %in% params$rct_ls_form_list) {
...
if (Sys.getenv('TIMCI_IS_RCT') == 1) {
  if (crf_day28_fid %in% params$rct_ls_form_list) {
...
if (wd_fid %in% params$rct_ls_form_list) {
...
if (spa_pid %in% params$odkc_project_list) {
...
if (cpid %in% params$odkc_project_list) {
  if (cgidi1_fid %in% ruODK::form_list(pid = cpid)$fid) {
    ...
    fn <- timci::export_df2xlsx(cgidi_invitation_data, params$qual1_dir, "01_timci_cg_invitation_data")
  }
}
```
...
if (cpid %in% params$odkc_project_list) {
  if (cgidi2_fid %in% ruODK::form_list(pid = cpid)$fid) {
    ...
    fn <- timci::export_df2xlsx(cgidi_encryption_data, params$qual1_dir, "02_timci_cg_encryption_data")
  }
}
```
...
if (cpid %in% params$odkc_project_list) {
  if (cgidi3_fid %in% ruODK::form_list(pid = cpid)$fid) {
    ...
    fn <- timci::export_df2xlsx(cgidi_interview_data, params$qual1_dir, "03_timci_cg_interview_data")
