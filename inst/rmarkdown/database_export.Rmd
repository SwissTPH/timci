...
cat(paste0(nrow(invalid_facilities), " records with non-valid enrolment facilities were found and removed in the locked Day 0 database."))
```
...
if (!is.null(facility_data)) {
  if (nrow(locked_day0_data) > 0) {
    if (nrow(raw_withdrawal_data) > 0) { 
      cat(paste0(nrow(raw_withdrawal_data), " withdrawals were reported (among which ", nrow(withdrawal_data2)," withdrawals with documented child IDs) and ", nrow(day0_data_id_withdrawal), " participants were found and removed in the locked Day 0 database."))
    } else {
...
```{r, results = 'asis'}
cat(paste0("* Raw Day 7 data are exported in ", day7_export_name, ".csv/xslx in the ", params$rctls_dir, " folder"))
```
...
```{r, results = 'asis'}
cat(paste0("* Raw sick child observation data are exported in ", spa_sco_export_name, ".csv/xslx in the ", params$spa_dir, " folder"))
```
