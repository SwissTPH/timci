...
### Non-valid enrolment facilities
...
```{r, results='asis'}
cat("Rule: no specific action at the moment.")
```
...
if(Sys.getenv('TIMCI_COUNTRY') != 'Kenya') {
    ...
    if (nrow(locked_pii) > 0) {
      ...
      if (nrow(duplicates) > 0) { 
        duplicates %>% knitr::kable(caption = "Possible name duplicates")
      }
    }
  }
  ...
  if (!is.null(facility_data)) {
    if (nrow(locked_pii) > 0) {
      if (nrow(duplicates) > 0) { 
        cat(paste0("", nrow(duplicates), " possible duplicated names were detected. No specific action was taken."))
      } else {
...
```{r, results='asis'}
cat("Rule: withdrawals with documented child IDs are removed from the locked database.")
```
  ...
  if (nrow(locked_day0_data) > 0) {
    if (!is.null(raw_withdrawal_data)) {
      if (nrow(raw_withdrawal_data) > 0) { 
        ...
        withdrawal_data2$child_id %>%  knitr::kable(col.names = c("child_id"))
      }
    }
  }
}
...
if (!is.null(facility_data)) {
  if (nrow(locked_day0_data) > 0) {
    if (!is.null(raw_withdrawal_data)) {
      if (nrow(raw_withdrawal_data) > 0) { 
        cat(paste0(nrow(raw_withdrawal_data), " withdrawals were reported (among which ", nrow(withdrawal_data2)," withdrawals with documented child IDs) and ", nrow(day0_data_id_withdrawal), " participants were automatically detected and removed in the locked Day 0 database."))
      } else {
...
```{r, results = 'asis'}
if (!is.null(day7fu_data)) {
  cat(paste0(nrow(day7fu_data_duplicates) - nrow(processed_day7fu_data_duplicates), " records were removed in the locked successful Day 7 follow-up database (records duplicated on the same day may be kept - we need to define a rule for this)."))
}
```
...
hospit_export_name <- "05_timci_followup_hospit_data"
if (!is.null(hospit_data)) {
...
if (!is.null(spa_sco_data)) {
  if (length(spa_sco_data) > 0) { 
    spa_sco_data_qc <- timci::detect_id_duplicates2(locked_spa_sco_data$'child_identification-pid')
...
if (!is.null(spa_hcpi_data)) {
  if (length(spa_hcpi_data) > 0) { 
    spa_hcpi_data_qc <- timci::detect_id_duplicates2(locked_spa_hcpi_data$hcp_id)
