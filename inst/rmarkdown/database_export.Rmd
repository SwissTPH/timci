...
cat("Rule: records from facility IDs not listed in the first section of this report are detected and corrected proactively were possible.")
```
...
**Definition 1:** Finalisation of the submission not done on the same day the submission was started (i.e., duration from start to end superior to 0 day)

```{r, results='asis', eval=!is.null(qc_completion_ab1day)}
cat(nrow(qc_completion_ab1day), "participant(s) for which the submission was not finalised on the same day it was started.")
```

**Definition 2:** Finalisation of the submission 2 days or later after the submission was started (i.e., duration from start to end superior to 1 day)

```{r, results='asis', eval=!is.null(qc_completion_ab2days)}
cat(nrow(qc_completion_ab2days), "participant(s) for which the submission was finalised after 2 days and more.")
```

```{r, results='asis', eval=!is.null(qc_completion_ab1day)}
qc_completion_fn <- "07_DQC1_nontimely_day0_completion"
fn <- timci::export_df2xlsx(qc_completion_ab1day,
                            qc_dir,
                            qc_completion_fn)
cat(paste0("The table of participants for which the submission was not finalised on the same day it was started is exported to ", qc_completion_fn))
```

```{r, results='asis', eval=!is.null(qc_completion_ab2days)}
if (nrow(qc_completion_ab2days) > 0) {
  hist(as.numeric(qc_completion_ab2days$diff),
...
```{r, results='asis', eval=!is.null(qc_transfer_ab12days)}
qc_transfer_fn <- "07_DQC2_nontimely_day0_transfer"
fn <- timci::export_df2xlsx(qc_transfer_ab12days,
                            qc_dir,
                            qc_transfer_fn)
cat(paste0("The table of participants for which the submission was transferred after 12 days and more (participant never displayed in the Day 7 follow-up log on the tablet) is exported to ", qc_transfer_fn))
```

```{r, results='asis', eval=!is.null(qc_transfer_ab12days)}
if (nrow(qc_transfer_ab12days) > 0) {
  hist(as.numeric(qc_transfer_ab12days$diff),
       xlab="Days between submission start and finalisation" ,
       ylab="Occurences",
       main="")
}
...
```{r}
write("Run Day 7 data quality checks", stderr())
```
...
```{r, results = 'asis'}
if (Sys.getenv('TIMCI_COUNTRY') == "Tanzania" || Sys.getenv('TIMCI_COUNTRY') == "India") {
  cat(paste0("* Locked Day 28 data are exported in ", day28_export_name, ".csv/xslx in the ", locked_db_dir, " folder"))
}
```
...
```{r, child = 'database_export_deaths_sub.Rmd'}
```
