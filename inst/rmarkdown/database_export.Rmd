...
### Non-valid device IDs

```{r}
# Added for India - check
screening_data <- timci::allocate_screening_facility(screening_data,
                                                     research_facilities)
...
```{r}
# Added for India - check
day0_data <- timci::allocate_screening_facility(day0_data,
                                                research_facilities)
```
...
```{r, results='asis'}
cat(paste0(nrow(invalid_facilities), " record(s) with non-valid enrolment facilities were automatically detected and removed in the locked Day 0 database."))
```
...
### Inconsistent facility information [Context check]

To be completed soon for India/Tanzania
...
### Submission timeliness [compliance check]

#### Non-timely completion
...
if (!is.null(qc_completion_ab2days)) {
  if (nrow(qc_completion_ab2days) > 0) {
    qc_completion_ab2days %>% knitr::kable(caption = "Submissions finalised after 2 days and more", row.names = FALSE)
  }
}
```
...
if (!is.null(qc_transfer_ab7days)) {
  if (nrow(qc_transfer_ab7days) > 0) {
    qc_transfer_ab7days %>% knitr::kable(caption = "Participants with a submission transfer after 7 days and more", row.names = FALSE)
  }
}
```
...
if (!is.null(qc_transfer_ab12days)) {
  if (nrow(qc_transfer_ab12days) > 0) {
    qc_transfer_ab12days %>% knitr::kable(caption = "Participants with a submissions transfer after 12 days and more", row.names = FALSE)
  }
}
```
...
Rule: child ID duplicates are removed from the locked database.
...
### Clinical presentation [Mandatory check check]

Description: all children should have an information about their clinical presentation.

Check to be implemented soon.
...
if (!is.null(allday7fu_data)) {
  if (nrow(allday7fu_data) > 0) {
    format_nrow("", allday7fu_data_uids, "records with non-valid participant IDs were automatically detected and removed in the locked Day 7 follow-up database.")
  }
}
```
...
if (!is.null(day7fu_data)) {
  if (nrow(day7fu_data) > 0) {
    format_nrow("", day7fu_data_uids, "records with non-valid participant IDs were automatically detected and removed in the locked successful Day 7 follow-up database.")
  }
}
```
...
```{r, results = 'asis'}
format_nrow("", hospit_data_uids, "records with non-valid participant IDs were automatically detected and removed in the locked hospitalisation database.")
```
...
```{r, results = 'asis'}
format_nrow("", hospit_data_duplicates, "duplicated IDs were automatically detected and removed in the locked hospitalisation database.")
```
...
`r if (Sys.getenv('TIMCI_COUNTRY') == "Tanzania" || Sys.getenv('TIMCI_COUNTRY') == "India"){'## Day 28 data quality checks'}`
...
```{r, results='asis'}
if (Sys.getenv('TIMCI_COUNTRY') == "Tanzania" || Sys.getenv('TIMCI_COUNTRY') == "India") {
  format_nrow("", allday28fu_data_uids, "records with non-valid participant IDs were automatically detected and removed in the locked Day 28 follow-up database.")
}
```

```{r, results='asis'}
if (Sys.getenv('TIMCI_COUNTRY') == "Tanzania" || Sys.getenv('TIMCI_COUNTRY') == "India") {
  format_nrow("", day28fu_data_uids, "records with non-valid participant IDs were automatically detected and removed in the locked successful Day 28 follow-up database.")
}
```
...
```{r timeflow-subsection, results="asis"}
if (!is.null(tf_data)){
...
```{r process-mapping-subsection, results="asis"}
if (!is.null(pm_data)){
...
```{r online-survey-export}
if (!is.null(online_survey_data)) {
