...
```{r, results='asis'}
cat("Rule: possible duplicates are detected based on name and date of birth. No specific action at the moment.")
```
```{r check-RCT-LS-ODK-facility-data-002, results='asis'}
if (Sys.getenv('TIMCI_COUNTRY') != 'Kenya') {
    if (!is.null(facility_data)) {
    if (nrow(locked_pii) > 0) {
      day0_qc1 <- timci::detect_namedob_duplicates(locked_pii)
      ...
      if (length(duplicates) > 0 & nrow(duplicates) > 0) { 
        duplicates %>% knitr::kable(caption = "Possible duplicates are detected based on the exact name and date of birth")
      }
    }
  }
}
...
if (!is.null(facility_data)) {
  if (nrow(locked_day0_data) > 0) {
    if (!is.null(raw_withdrawal_data)) {
      if (nrow(raw_withdrawal_data) > 0) { 
        cat(paste0(nrow(raw_withdrawal_data), " withdrawals were reported (among which ", nrow(withdrawal_data2)," withdrawals with documented child IDs) and ", nrow(day0_data_id_withdrawal), " participants were automatically detected."))
      } else {
