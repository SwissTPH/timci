...
```{r, results='asis', eval=(Sys.getenv('TIMCI_COUNTRY') == "Tanzania")}
...
```{r check-RCT-LS-ODK-facility-data-001a, results='asis', eval=(Sys.getenv('TIMCI_COUNTRY') == "Tanzania")}
day0_qc <- NULL
if (!is.null(facility_data)) {
  if (nrow(locked_day0_data) > 0) {
...
if (!is.null(facility_data)) {
  if (nrow(locked_day0_data) > 0) {
    if (nrow(id_duplicates) > 0) { 
      ...
      locked_day0_data_tmp <- locked_day0_data[!locked_day0_data$child_id %in% id_duplicates$child_id, ]
...
```{r, results='asis', eval=(Sys.getenv('TIMCI_COUNTRY') == "Tanzania")}
cat("##### Child ID duplicate edits")
```

```{r, results='asis',eval=(Sys.getenv('TIMCI_COUNTRY') == "Tanzania")}
out <- timci::correct_duplicates(locked_day0_data)
locked_day0_data <- out[[1]]
duplicate_edits <- out[[2]]
duplicate_edits %>% knitr::kable()
```

```{r, results='asis', eval=(Sys.getenv('TIMCI_COUNTRY') == "Tanzania")}
cat("##### Check after edits")
```
