...
```{r, results = 'asis'}
cat(paste0("There are ", nrow(locked_day7fu_data), " records in the locked Day 7 follow-up database."))
```
...
Rule: keep only IDs of children who are enrolled in the locked database.
...
## Sick child observation quality checks

```{r, results = 'asis'}
locked_spa_sco_data <- spa_sco_data %>% dplyr::filter(date <= as.Date(lock_date, "%Y-%m-%d"))
cat(paste0("There are ", nrow(locked_spa_sco_data), " records between ", start_date, " and ", lock_date, " in the raw SPA sick child observation database."))
```
...
```{r, results='asis'}
cat(paste0(nrow(spa_sco_data_uids), " records with non-valid participant IDs were automatically detected and removed in the locked SPA sick child observation database."))
```
