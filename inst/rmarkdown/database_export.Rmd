...
#print(logs)
project_list <- ruODK::project_list()$id
```
...
if (!is.null(facility_data)) { 
  ...
  timci::export_df2rds(screening_data, params$rctls_dir, "01_timci_screening_data")
}
```
...
if (!is.null(facility_data)) {
  ...
  timci::export_df2rds(deidentified_day0_data, params$rctls_dir, "02_timci_day0_data")
...
if (!is.null(facility_data)) {
  ...
  timci::export_df2xlsx(deidentified_facility_data, params$rctls_dir, "03_timci_facility_visits_data")
  timci::export_df2rds(deidentified_facility_data, params$rctls_dir, "03_timci_facility_visits_data")
}
```
...
if (!is.null(day7fu_data)) {
  ...
  tmp <- timci::export_df2rds(day7fu_data, params$rctls_dir, "04_timci_followup_day7_data")
}
```
...
if (!is.null(hospit_data)) {
  ...
  tmp <- timci::export_df2rds(hospit_data, params$rctls_dir, "05_timci_followup_hospit_data")
}
```
...
```{r load-SPA-environment-variables}
...
if (spa_pid %in% project_list) {
  if (cgei_fid %in% ruODK::form_list(pid = spa_pid)$fid) {
    ...
    timci::export_df2rds(spa_cgei_data, params$spa_dir, "03_timci_spa_exit_interview_data")
  }
}
```
...
if (spa_pid %in% project_list) {
  if (fa_fid %in% ruODK::form_list(pid = spa_pid)$fid) {
    ...
    timci::export_df2rds(spa_fa_data, params$spa_dir, "01_timci_spa_facility_assessment_data")
  }
}
```
...
if (spa_pid %in% project_list) {
  if (hcpi_fid %in% ruODK::form_list(pid = spa_pid)$fid) {
    ...
    timci::export_df2rds(spa_hcpi_data, params$spa_dir, "02_timci_spa_provider_interview_data")
  }
}
```
...
if (spa_pid %in% project_list) {
  if (sco_fid %in% ruODK::form_list(pid = spa_pid)$fid) {
    ...
    timci::export_df2rds(spa_sco_data, params$spa_dir, "04_timci_spa_consultation_obs_data")
  }
}
```
...
if (cpid %in% project_list) {
  if (cgidi1_fid %in% ruODK::form_list(pid = cpid)$fid) {
...
if (cpid %in% project_list) {
  if (cgidi2_fid %in% ruODK::form_list(pid = cpid)$fid) {
...
if (cpid %in% project_list) {
  if (cgidi3_fid %in% ruODK::form_list(pid = cpid)$fid) {
