...
Lock date: **`r lock_date`**

# 2. `r if (Sys.getenv('TIMCI_COUNTRY') == "Tanzania" || Sys.getenv('TIMCI_COUNTRY') == "India"){'Pragmatic cluster RCT'} else{'Longitudinal Observational Study'}`

## 2.1. Facility data quality checks
...
if (!is.null(facility_data)) {
  if (nrow(day0_data) > 0) {
    if (nrow(id_duplicates) > 0) { 
      ...
      locked_day0_data <- day0_data[!day0_data$child_id %in% id_duplicates$child_id, ]
...
if (!is.null(facility_data)) {
  if (nrow(day0_data) > 0) {
    if (nrow(raw_withdrawal_data) > 0) { 
      ...
      cat(paste0(nrow(raw_withdrawal_data), " withdrawals were reported and ", nrow(day0_data_id_withdrawal), " participants were removed from the database at Day 0."))
    } else {
...
```{r export-locked-screening-data}
if (!is.null(facility_data)) {
  locked_screening_data <- screening_data %>% dplyr::filter(date_visit <= as.Date(lock_date, "%Y-%m-%d"))
  locked_day0_data <- locked_day0_data %>% dplyr::filter(date_visit <= as.Date(lock_date, "%Y-%m-%d"))
...
Rule: only one successful follow-up is kept with the following priority rule: day 7 > day 8 > day 9 > day 10 > day 6 > day 11 and above
