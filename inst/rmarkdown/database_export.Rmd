...
```{r}
qc_nontimely_completion_id <- "DQC_00_01"
qc_nontimely_submission_id <- "DQC_00_02"
qc_duplicate_id1 <- "DQC_00_03a"
qc_duplicate_id2 <- "DQC_00_03b"
qc_multiple_enrolment_id <- "DQC_00_04"
qc_multiple_enrolment_sex_discrepancy_id <- "DQC_00_05"
qc_missing_cp_id <- "DQC_00_06"
qc_summary_id <- "DQC_00_07"
qc_nonvalid_day7fu <- "DQC_07_01"
qc_nonvalid_day28fu <- "DQC_28_01"
qc_nonreconciled_withdrawals_id <- "DQC_WD_01"
qc_unknown_spa_sco_patient_id <- "DQC_SPA_01"
```
...
facility_data_is_not_empty <- FALSE
if (facility_data_is_not_null) {
  facility_data_is_not_empty <- nrow(facility_data) > 0
}
```
...
```{r}
day7fu_is_not_empty <- !is.null(raw_day7fu_data)
if (day7fu_is_not_empty) {
  day7fu_is_not_empty <- nrow(raw_day7fu_data) > 0
}
```
...
```{r db-export-hospit-subsection, child = 'database_export_sub_rctls_hospit.Rmd'}
```

```{r}
day28fu_is_not_empty <- !is.null(raw_day28fu_data)
if (day28fu_is_not_empty) {
  day28fu_is_not_empty <- nrow(raw_day28fu_data) > 0
}
```

```{r db-export-day28fu-subsection, child = if (is_rct) 'database_export_sub_rctls_day28.Rmd'}
```

```{r db-export-withdrawals-subsection, child = 'database_export_sub_rctls_withdrawals.Rmd'}
```
