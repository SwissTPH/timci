  ...
  if (nrow(locked_day0_data) > 0) {
    if (!is.null(day0_data_id_withdrawal)) {
      if (nrow(day0_data_id_withdrawal) > 0) { 
        cat(paste0(nrow(day0_data_id_withdrawal), " participants were found in the locked Day 0 database."))
      }
    }
  }
}
...
```{r, results = 'asis'}
if (!is.null(pii) & !is.null(raw_day7fu_data) & !is.null(raw_hospit_data)) {
  hospit_fu <- timci::generate_hospital_log(pii = pii,
                                            fu7df = raw_day7fu_data,
                                            hospitdf = raw_hospit_data,
                                            deidentify = TRUE)
  timci::export_df2xlsx(hospit_fu,
                        params$rctls_dir,
                        "05b_timci_deidentified_hospit_fu_log_all")
}
```
