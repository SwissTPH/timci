...
if (!is.null(facility_data)) {
  if (nrow(locked_day0_data) > 0) {
    id_duplicates <- timci::identify_day0_id_duplicates_with_dates(locked_day0_data)
  }
}
```
...
day0_data_id_duplicates <- timci::deidentify_data(day0_data_id_duplicates)
```

```{r, results = 'asis'}
timci::quality_check_export(day0_data_id_duplicates,
                            qc_duplicate_id1,
                            "timci_day0_data_id_duplicates",
                            qc_dir,
                            "the submission was allocated the same ID that has been used for another participant in the databse")
```
...
```{r check-RCT-LS-ODK-facility-data-001b, results='asis'}
day0_qc <- NULL
if (!is.null(facility_data)) {
  if (nrow(locked_day0_data) > 0) {
    day0_qc <- timci::detect_id_duplicates(locked_day0_data)
