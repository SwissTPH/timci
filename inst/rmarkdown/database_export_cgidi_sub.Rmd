...
if (!is.null(day0_data) && !is.null(day7fu_data)) {
  cg_qual_export_data <- timci::generate_cg_log(locked_day0_data, locked_day7fu_data)
}
```
...
if (is_tanzania) {
  ...
  if (!is.null(cg_qual_export_data)) {
    cg_qual_export_data <- merge(x = cg_qual_export_data,
                                 ...
                                 by.y = 'facility_id',
                                 all = FALSE)
    cg_qual_export_data <- cg_qual_export_data %>% filter(intervention == 1 | intervention == 2)
  }
