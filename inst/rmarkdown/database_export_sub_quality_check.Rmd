...
```{r, eval=(qc_type=="duplicates")&!is.null(qc_df)}
uuid_cols <- grep("uuid", names(qc_df))
x <- unname((unlist(qc_df[colnames(qc_df[uuid_cols])])))
n_detected <- sum(!is.na(x)) / length(x)
```
