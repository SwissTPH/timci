...
```{r, eval=(qc_type=="nonvalid_ids_identify_duplicates")}
out <- timci::identify_nonvalid_ids_with_previous_duplicates(df,
                                                             idcol1,
                                                             refdf,
                                                             idcol2,
                                                             duplicatedf,
...
<!-- Detect matched names between follow-up and reference Day 0 database -->

```{r, eval=(qc_type=="matched_names")}
out <- timci::detect_matched_names_between_fu_and_day0(df,
                                                       refdf,
                                                       col_date,
                                                       col_name,
                                                       ldate_diff,
...
outcols <- c("date_visit", "child_id", "fid", "district", "fid_from_device")
if ( "fid_ra" %in%  cols ) {
...
outcols <- c(outcols, "uuid", "device_id")
qc_df <- qc_df %>%
  dplyr::select(outcols) %>%
  dplyr::arrange(fid)
```
...
outcols <- c("date_visit", "child_id", "fid", "district", "fid_from_device", "fid_ra", "district_ra")
if ( "child_id_manual" %in%  cols ) {
  outcols <- c(outcols, "child_id_manual")
}
if ( "latitude" %in%  cols ) {
