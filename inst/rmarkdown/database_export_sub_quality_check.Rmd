...
cols <- colnames(df)
outcols <- c("child_id", "fid", "fid_from_device")
if ( "fid_ra" %in%  cols ) {
...
cols <- colnames(df)
outcols <- c("child_id", "fid", "fid_from_device", "fid_ra")
if ( "latitude" %in%  cols ) {
  outcols <- c(outcols, "latitude")
}
if ( "longitude" %in%  cols ) {
...
qc_df <- df[(is.na(df$rx_amoxicillin) & !is.na(df$rx_othtype)) & (is.na(df$rx_amoxicillin_hf) & !is.na(df$rx_othtype_hf)),]
outcols <- c("child_id", "fid", "date_visit", "uuid")
```
...
drug_disp_df1 <- df %>%
  dplyr::filter(!!rlang::sym(col_id) != 96) %>%
  dplyr::select(free_text1, uuid) %>%
  dplyr::rename(free_text = free_text1)
drug_disp_df2 <- df %>%
  dplyr::filter(!!rlang::sym(paste0(col_id, "_hf")) != 96) %>%
  dplyr::select(free_text2, uuid) %>%
  dplyr::rename(free_text = free_text2)
qc_df <- rbind(drug_disp_df1,
               drug_disp_df2)
...
cols <- colnames(qc_df)
reformat_cond <- ( length(qc_df) > 5 ) & ( qc_type!="date_discrepancy" )
if ( reformat_cond ) {
