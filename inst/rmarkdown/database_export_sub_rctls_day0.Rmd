...
```{r}
qc_nontimelycompletion_id <- "DQC01_01"
qc_nontimelysubmission_id <- "DQC01_02"
```

#### Non-timely completion [compliance check `r qc_nontimelycompletion_id`]
...
#### Non-timely submission [compliance check `r qc_nontimelysubmission_id`]
...
timci::quality_check_export(qc_transfer_ab1day,
                            qc_nontimelysubmission_id,
                            "nontimely_day0_submission",
                            qc_dir,
                            "the submission was transferred strictly after the day it was finalised ")
```
...
**Description:** all participants should have complete information about their clinical presentation, i.e. we cannot have simultaneously: sx_convulsions = 0, sx_lethargy = 0, sx_vomit = 0, sx_less_feed = 0, sx_cough = 0, sx_difficulty_breath, sx_diarrhoea, sx_fever, sx_var = 96.
...
ts <- timci::export_df2xlsx(qc_missing_cp,
                            qc_dir,
                            qc_missing_cp_fn)
cat(paste0("The table of participants for whom the clinical presentation is missing has been exported (**", ts, "**) to **", qc_missing_cp_fn, ".xlsx**."))
```
...
```{r export-screening-data, results = 'asis', eval=!is.null(facility_data)}
timci::database_export(screening_data,
                       "01",
...
```{r export-locked-screening-data, results = 'asis', eval=!is.null(facility_data)}
timci::database_export(locked_screening_data,
                       "01",
                       "timci_screening_data",
                       locked_db_dir,
                       "Screening data")
...
```{r, results = 'asis'}
cat(paste0("* Raw Day 0 data are exported in ", "02_timci_day0_data", ".csv/xslx in the ", basename(params$rctls_dir), " folder"))
```

```{r, results = 'asis'}
cat(paste0("* Locked Day 0 data are exported in ", "02_timci_day0_data", ".csv/xslx in the ", basename(locked_db_dir), " folder"))
```
...
if (!is.null(facility_data)) {
  ...
  if (deidentification_on == 1) {
    ...
    ts <- timci::export_df2csvxlsx(deidentified_facility_data,
                                   params$rctls_dir,
                                  "03_timci_facility_visits_data")
  } else {
    ts <- timci::export_df2csvxlsx(study_data,
                                  params$rctls_dir,
                                  "03_timci_facility_visits_data")
  }
