...
```{r}
write(" o Discrepancy in facility IDs", stderr())
```

Facility information can be extracted in 4 different manners: from the research assistant's manual entry, from the first 5 characters of the child ID, from the device which is used (assuming 1 device is screening children in 1 unique facility per day) and from the GPS coordinates.
...
qc_export_description <- "facility information is inconsistent"
cat(knitr::knit_child('database_export_sub_quality_check.Rmd',
                      envir = environment(),
                      quiet = TRUE))
...
#### Spatiotemporal patterns of referrals [Context checks]
...
```{r, eval=timci::is_not_empty(day0_data)&!is_india, results='asis'}
fig_df <- day0_data %>%
  dplyr::mutate(week = as.Date(lubridate::floor_date(as.Date(start), "week", week_start = getOption("lubridate.week.start", 1)))) %>% 
