...
if (nrow(qc_completion_ab1day) > 0) {
  hist(as.numeric(qc_completion_ab1day$diff),
       breaks = c(1:max(as.numeric(qc_completion_ab1day$diff))),
...
fn <- timci::export_df2xlsx(qc_transfer,
                            qc_dir,
                            qc_transfer_fn)
...
```{r, results='asis', eval=!is.null(qc_transfer)}
if (nrow(qc_transfer) > 0) {
  hist(as.numeric(qc_transfer$diff),
       breaks = c(1:max(as.numeric(qc_transfer$diff))),
       xlab="Days between submission start and finalisation" ,
       ylab="Occurences",
       main="")
}
