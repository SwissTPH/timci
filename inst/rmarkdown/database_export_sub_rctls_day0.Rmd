...
Among the **`r n_cleaned_screening_records`** cleaned screening record(s), there are **`r n_raw_day0_records`** record(s) corresponding to new Day 0 enrolment(s) in the `r db_name` database.
...
```{r, eval=is_kenya&!is.null(edited_records)}
...
```{r, eval=is_kenya&!is.null(edited_records), results='asis'}
edited_records %>%
  dplyr::select(old_child_id, uuid, new_child_id) %>%
  knitr::kable()
```

`r if (is_kenya) { paste0('#### Final check after manual edits [compliance check', qc_nonvalid_fid2, ']\n\n') }`

```{r, eval=!is.null(day0_data), results='asis'}
qc_description <- "The facility ID (Fxxxx) is extracted from the participant enrolment ID (X-Fxxxx-Pxxxx). It should correspond to the ID of one of the research facility listed in Section 2.2 of this report."
...
```{r, eval=!is.null(day0_data)&is_tanzania, results='asis'}
...
```{r, eval=!is.null(day0_data)&(is_tanzania|is_kenya), results='asis'}
...
`r if ( is_tanzania ) { '#### Manual deletion of test/dummy data recorded as duplicated IDs\n\n' }`
...
`r if ( is_tanzania | is_kenya | is_senegal ) { '#### Manual correction of duplicated IDs\n\n' }`
...
`r if ( is_tanzania | is_kenya | is_senegal ) { paste0('#### Final check after manual edits [compliance check ', qc_duplicate_id2, ']\n\n') }`
...
`r if( !multiple) {'Only implemented for the final analysis - disabled for the interim analysis'}`
...
```{r, results='asis', eval=!is.null(day0_data)}
...
```{r}
day0_data <- timci::extract_pii(day0_data)[[1]]
