...
###########################
qc_inconsistent_facility_info1 <- "DQC_00_2ED_01"
qc_inconsistent_facility_info2 <- "DQC_00_2ED_02"
qc_nonvalid_fid1 <- "DQC_00_2ED_03"
qc_nonvalid_fid2 <- "DQC_00_2ED_04"
...
Facility information can be extracted in 4 different manners:

* from the research assistant's manual entry (variable *fid_ra*) - not implemented in Senegal ;
* from the first 5 characters of the child ID e.g., X-F0001-Pxxxx corresponds to facility F0001 (variable **fid**) ;
* calculation of the facility ID that is most used on 1 specific day with a specific device, assuming 1 device is used to screen children in 1 unique facility each day (derived variable **fid_from_device**)) ;
* from the tablet GPS coordinates - not implemented in Kenya.
...
#### Discrepancy in facility information [Logic/Context check `r qc_inconsistent_facility_info1`]

```{r}
write(" o Discrepancy in facility IDs (vs. device)", stderr())
```

`r if ( is_tanzania | is_senegal ) {'##### Initial check'}`

```{r, eval=facility_data_is_not_null, results='asis'}
qc_description <- "Check the facility ID from the first 5 characters of the child ID versus from the device which is used (assuming 1 device is screening children in 1 unique facility per day)."
qc_rule <- action_alert_no_modification
qc_type <- "inconsistent_facility_info1"
df <- day0_data
qc_text <- ""
qc_idx <- qc_inconsistent_facility_info1
qc_export_label <- "inconsistent_facility_info1_01"
...
Add the code of the closest facility

`r if ( is_tanzania ) {'##### Manual correction of child IDs'}`

Add a check to already test if a duplicate exists

```{r, eval=is_tanzania, results='asis'}
mc_description <- ""
to_mc_df <- day0_data
correction_type <- "edit_day0_child_ids_to_correct_facilities_from_check"
...
`r if ( is_senegal | is_tanzania ) {'* Manual correction of facility IDs'}`

```{r, eval=is_senegal|is_tanzania, results='asis'}
mc_description <- ""
to_mc_df <- day0_data
correction_type <- "correct_day0_inconsistent_facilities"
...
#### Discrepancy in facility information [Logic/Context check `r qc_inconsistent_facility_info2`]
...
qc_description <- "Check the facility ID from the first 5 characters of the child ID versus from the research assistant's manual entry."
qc_rule <- action_alert_no_modification
...
#### Manual corrections from the field

This includes information that cannot be detected (or very difficult to detect) using automated checks, for instance in the case QR codes were delivered to the wrong facility.

Only for Tanzania at the moment

```{r, eval=is_tanzania, results='asis'}
mc_description <- ""
to_mc_df <- day0_data
correction_type <- "edit_day0_child_ids_to_correct_facilities_from_field_info"
cat(knitr::knit_child('database_export_sub_corrections.Rmd',
                      envir = environment(),
                      quiet = TRUE))
day0_data <- mc_df
...
```{r}
write(" o Management of duplicated records", stderr())
```

```{r, eval=day0_is_not_null, results='asis'}
qc_description <- "Participants who have been entered twice as new enrolments on the same day, which creates duplicates in the database."
...
`r if ( !is_pilot ) { paste0('* Initial check ', qc_duplicate_id, 'a') }
...
`r if ( is_tanzania & !is_pilot ) { '#### Duplicated child IDs - Manual deletion of test/dummy data recorded as duplicated IDs\n\n' }`
...
`r if ( !is_pilot ) { '#### Manual correction of duplicated child IDs' }`
...
`r if( !multiple) {'Only implemented for the final analysis - checks disabled for the soft lock'}`
