...
fig_df <- day0_data %>%
  dplyr::mutate("Free text" = dplyr::case_when(
    !is.na(rx_othtype) & is.na(rx_othtype_hf) ~ "a) Free text from prescription only",
    is.na(rx_othtype) & !is.na(rx_othtype_hf) ~ "b) Free text from registry only",
    !is.na(rx_othtype) & !is.na(rx_othtype_hf) ~ "c) Free text from both sources",
    .default = "d) No free text entry")) %>%
  dplyr::mutate(week = lubridate::floor_date(as.Date(date_visit),
                                             "week",
                                             week_start = getOption("lubridate.week.start", 1)))

fig_caption <- "Free text entries"
facility_col <- "fid"
date_col <- "week"
date_lbl <- "Weeks"
date_format <- "%b%y"
...
fig_df <- day0_data %>%
  ...
  dplyr::mutate(Treatment = dplyr::case_when(
    !is.na(rx_amoxicillin) & is.na(rx_amoxicillin_hf) ~ "a) Structured entry from prescription only",
    is.na(rx_amoxicillin) & !is.na(rx_amoxicillin_hf) ~ "b) Structured entry from registry only",
    !is.na(rx_amoxicillin) & !is.na(rx_amoxicillin_hf) ~ "c) Structured entry from both sources",
    .default = "d) No structured entry"))
