...
qc_description <- "Facility information can be extracted in 4 different manners: from the research assistant's manual entry, from the first 5 characters of the child ID, from the device which is used (assuming 1 device is screening children in 1 unique facility per day) and from the GPS coordinates."
qc_rule <- action_alert_no_modification
qc_type <- "inconsistent_facility_info"
df <- day0_data
qc_text <- ""
qc_idx <- qc_inconsistent_facility_info
qc_export_label <- "inconsistent_facility_info"
qc_export_description <- "facility information is inconsistent"
cat(knitr::knit_child('database_export_sub_quality_check.Rmd',
                      envir = environment(),
                      quiet = TRUE))
n_inconsistent_facility_info <- n_detected
```
...
fig_df <- day0_data %>%
  dplyr::mutate("Free text" = dplyr::case_when(
    ...
    !is.na(rx_othtype_hf) & is.na(rx_othtype) ~ "b) Free text from registry only",
    !is.na(rx_othtype_hf) & !is.na(rx_othtype) ~ "c) Free text from both sources",
...
fig_df <- day0_data %>%
  ...
  dplyr::mutate(Treatment = dplyr::case_when(
    !is.na(rx_amoxicillin) ~ "Present",
    is.na(rx_amoxicillin) ~ "Missing"))
