...
fig_df <- day0_data %>%
  ...
  dplyr::mutate(Referral = dplyr::case_when(
    referral_cg == 1 ~ "Referred",
    referral_cg == 0 ~ "Not referred",
    referral_cg == 98 ~ "Not sure",
    referral_cg == 97 ~ "Declined",
    is.na(referral_cg) ~ "Missing")) %>%
  merge(research_facilities,
        by.x = 'fid',
        by.y = 'facility_id',
        all.x = TRUE)
...
```{r}
fig_caption <- "Missing prescription"
facility_col <- "fid"
date_col <- "week"
date_lbl <- "Weeks"
date_format <- "%b%y"
