...
`r if ( is_tanzania ) {'##### Child ID manual edits\n\n'}`
...
```{r}
edits_is_not_empty <- timci::is_not_empty(edited_records)
```

```{r, eval=is_tanzania&edits_is_not_empty, results='asis'}
edited_records %>%
  dplyr::select(old_child_id, uuid, new_child_id) %>%
  knitr::kable()
```
...
qc_text <- ""
qc_idx <- qc_inconsistent_facility_info1
qc_export_label <- "inconsistent_facility_info1_after_childID_correction"
qc_export_description <- "facility information between the child ID and the device is inconsistent"
cat(knitr::knit_child('database_export_sub_quality_check.Rmd',
                      envir = environment(),
                      quiet = TRUE))
...
qc_text <- ""
qc_idx <- qc_inconsistent_facility_info2
qc_export_label <- "inconsistent_facility_info2_after_childID_correction"
qc_export_description <- "facility information between the child ID and the research assistant's entry is inconsistent"
cat(knitr::knit_child('database_export_sub_quality_check.Rmd',
                      envir = environment(),
                      quiet = TRUE))
...
`r if ( is_senegal | is_tanzania ) {'##### Facility ID manual edits\n\n'}`
...
```{r, eval=(is_senegal)&!is.null(edited_records)}
edited_df_status_update <- paste0('**', nrow(edited_records), '** record(s) were manually edited in the ', db_name, ' database.')
```

`r edited_df_status_update`

```{r, eval=(is_senegal)&!is.null(edited_records), results='asis'}
edited_records %>%
  dplyr::select(child_id, uuid, new_fid) %>%
  knitr::kable()
```

```{r, eval=facility_data_is_not_null&(is_senegal|is_tanzania), results='asis'}
qc_description <- "Check the facility ID from the first 5 characters of the child ID versus from the device which is used (assuming 1 device is screening children in 1 unique facility per day)."
qc_rule <- action_alert_no_modification
qc_type <- "inconsistent_facility_info1"
df <- day0_data
qc_text <- ""
qc_idx <- qc_inconsistent_facility_info1
qc_export_label <- "inconsistent_facility_info1_after_facilityID_correction"
qc_export_description <- "facility information between the child ID and the device is inconsistent"
cat(knitr::knit_child('database_export_sub_quality_check.Rmd',
                      envir = environment(),
                      quiet = TRUE))
...
qc_text <- ""
qc_idx <- qc_inconsistent_facility_info2
qc_export_label <- "inconsistent_facility_info2_after_facilityID_correction"
qc_export_description <- "facility information between the child ID and the research assistant's entry is inconsistent"
cat(knitr::knit_child('database_export_sub_quality_check.Rmd',
                      envir = environment(),
                      quiet = TRUE))
...
#### Duplicated records [Compliance check `r qc_true_duplicates`]
...
`r if ( !is_pilot ) { paste0('#### Duplicated child IDs - Initial check [Compliance check ', qc_duplicate_id1, ']') }`
...
`r if ( !is_pilot ) { '#### Duplicated child IDs - Manual correction of duplicated IDs\n\n' }`
...
`r if ( !is_pilot ) { paste0('#### Duplicated child IDs - Final check after manual edits [Compliance check ', qc_duplicate_id2, ']') }`
...
```{r}
fig_caption <- "Duplicated IDs (after manual edits)"
facility_col <- "fid"
date_col <- "week"
date_lbl <- "Weeks"
date_format <- "%b%y"
