...
### Penicillin G entries [Expert knowledge checks `r qc_penig_consistency_drug`]

```{r}
write("\to Penicillin G check", stderr())
```

```{r, results='asis', eval=drug_is_not_null}
qc_description <- "All free entries should contain Penicillin G"
qc_rule <- action_alert_no_modification
qc_type <- "drug_reentry_accuracy"
df <- drug_data
col_id <- "rx_penicillinG"
cleaning <- "none"
qc_text <- "Penicillin G entries"
qc_idx <- qc_penig_consistency_drug
qc_export_label <- "penig_entries"
qc_export_description <- "Penicillin G was re-entered in a structured manner"
cat(knitr::knit_child('database_export_sub_quality_check.Rmd',
                      envir = environment(),
                      quiet = TRUE))
