...
There are **`r if ( !is.null(allday28fu_data) ) { nrow(allday28fu_data) } else { 'NA' }`** records in the raw Day 28 follow-up database, among which **`r if ( !is.null(day28fu_data) ) { nrow(day28fu_data) } else { 'NA' }`** records documenting successful follow-ups.
