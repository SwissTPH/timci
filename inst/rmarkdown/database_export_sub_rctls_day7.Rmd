...
There are **`r nrow(allday7fu_data)`** records in the raw Day 7 follow-up database, among which **`r nrow(day7fu_data)`** records documenting successful Day 7 follow-ups.
...
* **`r if ( !is.null(allday7fu_data_uids) ) { nrow(allday7fu_data_uids) } else {'NA'}`** records with non-valid participant IDs detected and removed in the locked Day 7 follow-up database.

* **`r if ( !is.null(day7fu_data_uids) ) { nrow(day7fu_data_uids) } else {'NA'}`** records with non-valid participant IDs detected and removed in the locked successful Day 7 follow-up database.
...
```{r check-RCT-LS-ODK-fu-day7-data}
...
```{r, results='asis', eval=timci::is_not_empty(day7fu_duplicates)}
cat(paste0("**", nrow(day7fu_duplicates), "** duplicated successful follow-ups detected."))
```

```{r, eval=timci::is_not_empty(locked_day7fu_data)}
id_duplicates <- timci::identify_dayfu_id_duplicates_with_dates(locked_day7fu_data)
```

```{r, results = 'asis'}
timci::quality_check_export(id_duplicates,
                            qc_duplicated_day7fu,
                            "duplicated_successful_day7fu",
                            qc_dir,
                            "Day 7 follow-ups are duplicated")
```
...
**Rule:** only the most recent successful Day 7 follow-up is kept.
