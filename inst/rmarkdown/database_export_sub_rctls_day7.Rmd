...
#### Follow-ups corresponding to participants enrolled in the CDSA pilot (India only)
...
allday7fu_data <- timci::correct_day7_all(allday7fu_data)
```

##### Final check

```{r}
write(" o Non-valid participant IDs (final check)", stderr())
```
...
qc_idx <- qc_nonvalid_day7fu
qc_export_label <- "nonvalid_pids_day7fu__with_matched_names_after_edits"
qc_export_description <- "the child ID does not correspond to any ID found in the locked Day 0 database"
cat(knitr::knit_child('database_export_sub_quality_check.Rmd',
                      envir = environment(),
                      quiet = TRUE))
