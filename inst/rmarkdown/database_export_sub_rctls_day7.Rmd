...
```{r}
day7fu_pii_drops <- c("name", "cg_name", "phone_nb")
```
...
refdf <- facility_data
idcol2 <- "child_id"
duplicatedf <- initial_duplicates_df
idcol3 <- "id"
valcol <- "is_duplicate"
...
qc_description <- "Day 7 follow-ups are relevant only if they can be reconciled with a participant enrolled at Day. Non-reconciled Day 7 follow-ups are searched for matched names."
qc_rule <- "Keep only IDs of children who can be found in the initial Day 0 database."
...
qc_idx <- qc_nonconsistent_name_day7fu
qc_export_label <- "nonconsistent_names_day7fu"
