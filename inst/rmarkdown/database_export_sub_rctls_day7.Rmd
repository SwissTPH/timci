...
```{r, results = 'asis'}
cat(paste0("There are **", nrow(allday7fu_data), "** records in the raw Day 7 follow-up database, among which **", nrow(day7fu_data), "** records documenting successful Day 7 follow-ups."))
```
...
ts <- timci::export_df2xlsx(allday7fu_data_uids[c("child_id", "uuid")],
                            qc_dir,
                            allday7fu_data_uids_fn)
cat(paste0("The table of Day 7 follow-ups for which the child ID does not correspond to any ID found the locked Day 0 database has been exported (**", ts,"**) to **", allday7fu_data_uids_fn, ".xlsx**."))
```
...
```{r, results='asis', eval=!is.null(id_duplicates)}
duplicated_day7fu_ids_fn <- "DQC_07_01_duplicated_successful_day7fu"
ts <- timci::export_df2xlsx(id_duplicates,
                            qc_dir,
                            duplicated_day7fu_ids_fn)
cat(paste0("The table of Day 7 follow-ups that are duplicated has been exported (**", ts,"**) to **", duplicated_day7fu_ids_fn, ".xlsx**."))
```
