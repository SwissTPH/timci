...
### Data entry times [Context check]

```{r, results='asis', eval=timci::is_not_empty(facility_data), fig.asp=1, fig.cap="Spatiotemporal pattern of data entry times"}
ggplot2::ggplot(facility_data %>%
                  dplyr::mutate(start_time1 = as.POSIXct(paste0(Sys.Date(), df$start_time), tz = Sys.getenv("TZ"))) %>% 
