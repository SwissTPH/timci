...
qc_root <- "DQC_00_0SG"
qc_incorrect_date_setup <- paste(qc_root, "01", sep = "_")
qc_nontimely_submission_id <- paste(qc_root, "02", sep = "_")
qc_nontimely_completion_id <- paste(qc_root, "03", sep = "_")
qc_screening_before_startdate <- paste(qc_root, "04", sep = "_")
qc_screening_after_lockdate <- paste(qc_root, "05", sep = "_")
qc_screening_w_nonvalid_deviceid <- paste(qc_root, "06", sep = "_")
qc_screening_before_facility_startdate <- paste(qc_root, "07", sep = "_")
qc_nonvalid_fid <- paste(qc_root, "08", sep = "_")
qc_screening_other_fids <- paste(qc_root, "09", sep = "_")
qc_inconsistent_facility_info1 <- paste(qc_root, "10", sep = "_")
qc_inconsistent_facility_info2 <- paste(qc_root, "11", sep = "_")
qc_inconsistent_facility_info3 <- paste(qc_root, "12", sep = "_")
qc_inconsistent_age_info <- paste(qc_root, "13", sep = "_")
qc_underaged_cg_id <- paste(qc_root, "14", sep = "_")
qc_dup_repeats_id <- paste(qc_root, "15", sep = "_")
qc_other_repeats_id <- paste(qc_root, "16", sep = "_")
```
...
n_before_facility_startdate_records <- 0
...
Initial check `r qc_dup_repeats_id`a
:::
...
qc_idx <- paste0(qc_inconsistent_facility_info1, "z")
...
```{r}
screening_data_nopseudo <- timci::extract_screening_data(facility_data,
                                                         is_pilot) %>%
  dplyr::select(-fid_from_device)
```

```{r}
screening_data <- screening_data_nopseudo %>%
  dplyr::rowwise() %>%
...
```{r export-locked-screening-data-no-pseudonymisation, results = 'asis'}
timci::dataset_export(screening_data_nopseudo,
                      "01",
                      "timci_screening_data_without_pseudonymisation",
                      rctls_dir,
