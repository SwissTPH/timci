...
```{r, results='asis'}
qc_description <- "The enrolment date is defined as the creation (start) date of the form. However if for any reason the date is not setup correctly on the tablet, the enrolment date is not correct and follow-up is not triggered on the right date. If the submission date is anterior to the creation date in the database."
qc_rule <- "Incorrect creation date is replaced by the submission date"
