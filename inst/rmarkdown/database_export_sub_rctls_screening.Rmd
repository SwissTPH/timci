...
qc_root <- "DQC_00_0SG"
qc_incorrect_date_setup <- paste(qc_root, "01")
qc_nontimely_submission_id <- paste(qc_root, "02")
qc_nontimely_completion_id <- paste(qc_root, "03")
qc_screening_before_startdate <- paste(qc_root, "04")
qc_screening_after_lockdate <- paste(qc_root, "05")
qc_screening_w_nonvalid_deviceid <- paste(qc_root, "06")
qc_screening_before_facility_startdate <- paste(qc_root, "07")
qc_nonvalid_fid <- paste(qc_root, "08")
qc_screening_other_fids <- paste(qc_root, "09")
qc_inconsistent_facility_info1 <- paste(qc_root, "10")
qc_inconsistent_facility_info2 <- paste(qc_root, "11")
qc_inconsistent_facility_info3 <- paste(qc_root, "12")
qc_inconsistent_age_info <- paste(qc_root, "13")
qc_underaged_cg_id <- paste(qc_root, "14")
qc_dup_repeats_id <- paste(qc_root, "15")
qc_other_repeats_id <- paste(qc_root, "16")
```
