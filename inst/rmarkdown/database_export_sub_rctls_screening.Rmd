...
ggplot2::ggplot(qc_df %>% 
                  dplyr::mutate(Status = ifelse(is.na(fid), "Not enrolled", "Enrolled")) %>%
                  dplyr::mutate(week = lubridate::floor_date(as.Date(end),
                                                             "week",
                                                             week_start = getOption("lubridate.week.start", 1))),
                aes(x = week, fill = Status)) +
  ggplot2::geom_bar() +
  ggplot2::labs(x = "Weeks",
                y = "Number of submissions not transferred on the same day they were finalised") +
  ggplot2::facet_wrap(~ fid_from_device, ncol = 5) +
  ggplot2::theme(text = element_text(size = tsize, family = tfont),
                 axis.ticks.x=element_blank())
```

```{r, results='asis', eval=timci::is_not_empty(qc_df), fig.asp=1}
ggplot2::ggplot(qc_df %>% 
                  dplyr::mutate(Status = ifelse(diff < 3, "< 3 days", ">= 3 days")) %>%
                  dplyr::mutate(week = lubridate::floor_date(as.Date(end),
                                                             "week",
                                                             week_start = getOption("lubridate.week.start", 1))),
                aes(x = week, fill = Status)) +
...
ggplot2::ggplot(qc_df %>% 
  ...
  ggplot2::geom_bar() +
  ggplot2::labs(x = "Weeks",
                y = "Number of submissions not finalised on the same day they were started") +
  ggplot2::facet_wrap(~ fid_from_device, ncol = 5) +
  ggplot2::theme(text = element_text(size = tsize, family = tfont),
                 axis.ticks.x=element_blank())
```
...
Add a faceted graph of screening times by facility overtime

```{r, results='asis', fig.asp=1}
ggplot2::ggplot(facility_data %>%
                  dplyr::mutate(start_time1 = lubridate::hm(start_time)) %>% 
                  dplyr::mutate(week = lubridate::floor_date(as.Date(end),
                                                             "week",
                                                             week_start = getOption("lubridate.week.start", 1))),
                aes(x = week, y = start_time1)) +
  ggplot2::geom_point() +
  ggplot2::lims(y = c(lubridate::hm('00:00'),lubridate::hm('23:59'))) +
  ggplot2::facet_wrap(~ fid_from_device, ncol = 5) +
  ggplot2::theme(text = element_text(size = tsize, family = tfont),
                 axis.ticks.x=element_blank())
```
...
ggplot2::ggplot(screening_data %>% 
  ...
  ggplot2::geom_bar() +
  ggplot2::labs(x = "Weeks",
                y = "Number of screening submissions") +
  ggplot2::facet_wrap(~ fid_from_device, ncol = 5) +
  ggplot2::theme(text = element_text(size = tsize, family = tfont)) 
```
