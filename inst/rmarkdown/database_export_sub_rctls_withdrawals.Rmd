...
if (nrow(locked_day0_data) > 0) {
  if (!is.null(raw_withdrawal_data)) {
    if (nrow(raw_withdrawal_data) > 0) {
      ...
      non_reconciled_withdrawn_ids <- withdrawal_data2[withdrawal_data2$child_id %!in% locked_day0_data$child_id, ]
...
if (!is.null(raw_withdrawal_data)) {
  ...
  cat(paste0("* Raw withdrawal data are exported (**", ts, "**) in **", withdrawal_export_name, ".csv/xslx** in the **", basename(params$rctls_dir), "** folder"))
}
```
...
if (!is.null(raw_withdrawal_data)) {
  if (!is.null(day0_data_id_withdrawal)) {
    ts <- timci::export_df2csvxlsx(day0_data_id_withdrawal,
                                   locked_db_dir,
                                   withdrawal_export_name)
    cat(paste0("* Locked withdrawal data are exported (**", ts, "**) in **", withdrawal_export_name, ".csv/xslx** in the **", basename(locked_db_dir), "** folder"))
  }
}
```
