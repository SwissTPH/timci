...
failed_day7fu_data <- timci::format_day7_data(raw_day7fu_data)[[2]]
...
```{r}
library(magrittr)
url <- "https://timicodktest.smartforest.de"
res <- httr::RETRY("GET",
                   paste0(url, "/v1/projects/14/forms/01c-TIMCI-CRF-Day7"),
                   httr::authenticate(Sys.getenv("ODKC_UN"), Sys.getenv("ODKC_PW"))) %>% httr::content(.)
cversion <- as.numeric(res$version)
print(cversion)
```
...
url <- "https://timicodktest.smartforest.de"
res <- httr::RETRY("POST",
                   paste0(url, "/v1/projects/14/forms/01c-TIMCI-CRF-Day7/draft"),
                   httr::authenticate(Sys.getenv("ODKC_UN"), Sys.getenv("ODKC_PW"))) %>% httr::content(.)
print(res)
```
...
library(magrittr)
url <- "https://timicodktest.smartforest.de"
f <- system.file(file.path('extdata', "day7fu_test.csv"), package = 'timci')
res <- httr::RETRY("POST",
                   paste0(url, "/v1/projects/14/forms/01c-TIMCI-CRF-Day7/draft/attachments/day7fu.csv"),
...
```{r}
library(magrittr)
nversion <- as.character(cversion + 0.00001)
print(nversion)
url <- "https://timicodktest.smartforest.de"
res <- httr::RETRY("POST",
                   paste0(url, "/v1/projects/7/forms/08-TIMCI-cg-idi-main2/draft/publish?version=",nversion),
                   httr::authenticate(Sys.getenv("ODKC_UN"), Sys.getenv("ODKC_PW"))) %>% httr::content(.)
print(res)
```
