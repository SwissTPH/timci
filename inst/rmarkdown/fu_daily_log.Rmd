...
physicalday7fu_start <- params$physicalday7fu_start
...
fu <- timci::generate_fu_log_csv(pii,
                                 ...
                                 fu_vstart,
                                 fu_vend)
fn <- timci::export_df2xlsx(fu,
                            getwd(),
                            "follow-up_test")
```

```{r, eval=(Sys.getenv('TIMCI_COUNTRY') == 'Tanzania')&(fu_fid == Sys.getenv('TIMCI_CRF_DAY7_FID'))}
physicalday7fu <- timci::generate_physical_fu_log_csv(pii = pii,
                                                      fudf = raw_fu_data,
                                                      wmin_nophone = physicalday7fu_start,
                                                      wmin_phone = physicalday7fu_start,
                                                      wmax = physicalday7fu_end,
                                                      vwmin = fu_vstart,
                                                      vwmax = fu_vend)
```
...
```{r, eval=(fu_fid == Sys.getenv('TIMCI_CRF_DAY7_FID'))}
...
write.csv(physicalday7fu, physicalfu_csv_filename, row.names = FALSE, quote = FALSE, fileEncoding = "UTF-8")
```

```{r, eval=(params$fu_fid == Sys.getenv('TIMCI_CRF_DAY28_FID'))}
...
```{r, eval=(fu_fid %in% rct_ls_form_list)&(Sys.getenv('TIMCI_COUNTRY') == 'Tanzania')&(fu_fid == Sys.getenv('TIMCI_CRF_DAY7_FID'))}
# Upload the new CSV for physical follow-up created as a form attachment
physicalfu_csv_upload_response <- timci::upload_odkc_csv_attachment(ruODK::get_default_un(), ruODK::get_default_pw(), form_url, physicalfu_csv_filename)
