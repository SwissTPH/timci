...
```{r maps-plots-india-maps, eval=is_india, fig.cap=fcap_up_maps}
# Set plot layout
...
```{r maps-clear-india-memory, eval=is_india}
rm(up_districts)
rm(deoria_dc)
rm(sitapur_dc)
rm(unnao_dc)
...
```{r maps-loads-kenya-data, eval=is_kenya}
...
```{r maps-plots-kenya-maps, eval=is_kenya, fig.cap="Kenya maps"}
# Set plot layout
layout(mat = matrix(c(1, 1, 1,
                      1, 1, 1,
                      2, 3, 4,
...
```{r maps-clear-kenya-memory, eval=is_kenya}
rm(kenyan_counties)
rm(kakamega)
rm(uasingishu)
rm(kitui)
...
```{r maps-loads-senegal-data, eval=is_senegal}
...
```{r maps-plots-senegal-map, eval=is_senegal, fig.cap="Région de Thiès et coordonnées GPS des screenings"}
# Plot: Senegal map
graphics::par(mar = c(0, 0, 0, 0))
...
```{r maps-clear-senegal-memory, eval=is_senegal}
rm(senegal_regions)
rm(thies)
tmp <- gc() # garbage collection
```
