...
if (!is.null(wfa_data)) {
  ...
  if (nrow(wfa_data) > 0) {
    ...
    last_data2 <- timci::extract_last_fa_data2(wfa_data, stats)
    tmp <- timci::export_df2xlsx(format_weekly_fa_data_for_export(last_data2), path_dir, paste0("TIMCI_M&E_RA_data_for_PATH", Sys.Date()))
