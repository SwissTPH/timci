...
### Select only participants who have a SpO2 value
```{r, echo=TRUE, message=FALSE}
df1 <- df %>% dplyr::filter(spo2 == 1)
df %>%
  head(10) %>%
  knitr::kable()
print(nrow(df))
...
### Select only participants who have a phone number available
```{r, echo=TRUE, message=FALSE}
df1 <- df %>% dplyr::filter(phone_nb_avail == 1)
df %>%
  head(10) %>%
  knitr::kable()
print(nrow(df))
