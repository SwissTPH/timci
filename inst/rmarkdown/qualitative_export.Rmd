...
sl <- ruODK::submission_list(pid = 12,
                             fid = "08c-TIMCI-cg-idi-interview")
for (i in nrow(sl)) {
  uuid <- sl$instance_id[[i]]
  sub <- ruODK::get_one_submission(pid = 12,
                                   fid = "08c-TIMCI-cg-idi-interview",
                                   uuid)
  sub2 <- unlist(sub)
  print(sub2)
  cpath <- file.path(params$qual_dir, sub$identification$idiidm)
  dir.create(cpath, showWarnings = FALSE)
  tmp <- timci::export_df2xlsx(sub2, cpath, "data")
