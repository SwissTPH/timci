...
if (nrow(facility_data) > 0) {
  ...
  plot2 <- timci::plot_enrolment_gauge(n_enrolled/(days_nb*8*nb_facilities), plot2_str, 100, 62.5, 100)
}
```
...
```{r, results = "asis"}
facility_data$timestamp <- strptime(facility_data$start, format = "%H:%M:%S")
facility_data$hours <-  as.numeric(format(facility_data$timestamp, format = "%H"))
hist(facility_data$hours,
...
age_statistics <- list(
  "Facility" = list(
    "Above 59 months" = ~qwraps2::n_perc(age_incl == 0, dec_pres),
    "2 - 59 months" = ~qwraps2::n_perc(age_incl == 1 & yg_infant == "2-59m", dec_pres),
    "0 - 2 months" = ~qwraps2::n_perc(age_incl == 1 & yg_infant == "0-2m", dec_pres),
    "First day of life" = ~qwraps2::n_perc(age_excl == 1, dec_pres)
    )
)
...
sickness_statistics <- list(
  "Facility" = list(
    "Sick children" = ~qwraps2::n_perc(sickness == 1, dec_pres),
    "Other children" = ~qwraps2::n_perc(sickness == 0, dec_pres)
    )
)
...
sick_reason_statistics <- list(
  "Facility" = list(
    "Sickness" = ~qwraps2::n_perc(consult_reason == 1, dec_pres),
    "Trauma" = ~qwraps2::n_perc(consult_reason == 2 & sickness == 1, dec_pres),
    "Immunisation" = ~qwraps2::n_perc(consult_reason == 3 & sickness == 1, dec_pres),
    "Monitoring" = ~qwraps2::n_perc(consult_reason == 4 & sickness == 1, dec_pres)
    )
)
...
reason_statistics <- list(
  "Facility" = list(
    "Trauma" = ~qwraps2::n_perc(consult_reason == 2 & sickness == 0, dec_pres),
    "Immunisation" = ~qwraps2::n_perc(consult_reason == 3 & sickness == 0, dec_pres),
    "Monitoring" = ~qwraps2::n_perc(consult_reason == 4 & sickness == 0, dec_pres),
    "Inpatient admission" = ~qwraps2::n_perc(inpatient == 1, dec_pres)
    )
)
...
summary_statistics <- list(
  "Facility" = list(
    "Below 18 years old" = ~qwraps2::n_perc(cg_eligibility == 0, dec_pres),
    "Above 18 years old" = ~qwraps2::n_perc(cg_eligibility == 1, dec_pres)
    )
)
...
mother_statistics <- list(
  "Mother's age" = list(
    "Below 18" = ~qwraps2::n_perc(underaged_mother == 1 | (main_cg == 1 & cg_eligibility == 0), dec_pres),
    "Above 18" = ~qwraps2::n_perc(underaged_mother == 0 | (main_cg == 1 & cg_eligibility == 1), dec_pres)
    )
)
...
summary_statistics <- list(
  "Facility" = list(
    "First" = ~qwraps2::n_perc(prev_enrl == 3 | prev_enrl == 98, dec_pres),
    "Previous" = ~qwraps2::n_perc(prev_enrl == 1 | prev_enrl == 2, dec_pres),
    "Last <= 28 days" = ~qwraps2::n_perc(repeat_consult == 1, dec_pres),
    "Last > 28 days" = ~qwraps2::n_perc((prev_enrl == 1 | prev_enrl == 2) & repeat_consult == 0, dec_pres)
    )
)
st <- qwraps2::summary_table(all, summary_statistics)
st1 <- qwraps2::summary_table(dplyr::group_by(all, facility_name), summary_statistics)
...
consent_statistics <- list(
  "Facility" = list(
    "Consented" = ~qwraps2::n_perc(consent == 1, dec_pres),
    "Not consented" = ~qwraps2::n_perc(consent == 0, dec_pres)
    )
)
