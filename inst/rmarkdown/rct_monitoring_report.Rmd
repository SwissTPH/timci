...
timci::generate_calendar_heatmap(facility_data,
                                 date_visit,
...
if (nrow(baseline_data) > 0) {
  timci::generate_calendar_heatmap(baseline_data,
                                   date_visit,
                                   end_date = as.Date(end_date))
}
```
...
age_statistics <- list(
  "Facility" = list(
    ...
    "1 - 59 days" = ~qwraps2::n_perc(age_excl == 0 & age_incl == 1 & yg_infant == "0-2m", pres),
...
if (!is.null(facility_data)) {
  if (nrow(facility_data) > 0) {
    ...
    phone_stats <- list(
      "Facility" = list(
        "0 phone numbers" = ~qwraps2::n_perc(phone_nb == 0, 2),
        "1 phone number" = ~qwraps2::n_perc(phone_nb == 1, 2),
        "2 phone numbers" = ~qwraps2::n_perc((phone_nb == 1) & (phone_nb2 == 1), 2)
        )
    )
...
if (nrow(baseline_data) > 0) {
  ...
  #print(format_summary_table(all, summary_statistics, yg_infant))
}
```
...
if (!is.null(raw_day7fu_data)) {
  if (nrow(raw_day7fu_data) > 0) {
    plot(timci::generate_day_bar_plot(raw_day7fu_data$date,
                                      as.Date(Sys.Date() - w),
                                      as.Date(Sys.Date() + 1),
                                      ylbl = submission_str))
  } else{
...
if (!is.null(raw_day7fu_data)) {
  if (nrow(raw_day7fu_data) > 0) {
    timci::generate_calendar_heatmap(raw_day7fu_data,
                                     date)
  } else{
    cat('N/A\n\n')
  }
...
if (!is.null(day7fu_data)) {
  if (nrow(day7fu_data) > 0) {
    ...
    day7_call_stats <- list(
      "District" = list(
        "Successful call" = ~qwraps2::n_perc(cg_reached == 1, 2),
  ...
  if (nrow(day7fu_data) > 0) {
    ...
    day7_outcome_ss <- list(
      "District" = list(
        ...
        "Death" = ~qwraps2::n_perc(status_day7 == 3, 2),
        "Higher level visit" = ~qwraps2::n_perc(status_day7 == 2 | (status_day7 != 2 & hf_visit_day7 == 1 & hf_visit_type == 1), 2)
        )
      )
...
if (Sys.getenv('TIMCI_COUNTRY') == "Tanzania" || Sys.getenv('TIMCI_COUNTRY') == "India") {
  ...
  if (!is.null(raw_day28fu_data)) {
    if (nrow(raw_day28fu_data) > 0) {
      timci::generate_calendar_heatmap(raw_day28fu_data,
                                       date)
    } else{
      cat('N/A\n\n')
    }
