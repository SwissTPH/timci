  ...
  if (nrow(pii) > 0) {
    ...
    out %>% 
      kableExtra::kbl(booktabs = TRUE,
                      longtable = TRUE,
                      linesep = "",
                      caption = "Number of unique phone numbers by facility") %>% 
      kableExtra::kable_styling(latex_options = c("HOLD_position", "repeat_header"), font_size = table_fs) %>%
      kableExtra::row_spec(0, bold = TRUE) %>%
      kableExtra::row_spec(1:(nrow(out)/2) * 2, background = striped_row_bg)
...
pox_statistics <- list(
  "Facility" = list(
    "1 - 59 days" = ~qwraps2::n_perc(spo2_yg_infant == 1,
                                     prec,
                                     na_rm = TRUE),
    "2-59 months" = ~qwraps2::n_perc(spo2_older_children == 1,
                                     prec,
                                     na_rm = TRUE)
    )
)
