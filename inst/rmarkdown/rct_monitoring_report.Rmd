...
if (Sys.getenv('TIMCI_COUNTRY') == 'Senegal') {
  ...
  baseline_str <- "Visite primaire (Jour 0)"
  ...
  baseline_str <- "Baseline visit (day 0)"
...
pii <- merge(pii, params$research_facilities[, c("deviceid", "facility_name")], by.x = 'device_id', by.y = 'deviceid', all.x = TRUE)
...
facility_disp  %>% 
  kableExtra::kbl(booktabs = TRUE, linesep = "", caption = facility_cap) %>% 
...
```{r, results='asis'}
if (n_due_day7fu > 0) {
  plot4 <- timci::plot_gauge(n_completed_day7fu/n_due_day7fu, plot4_str, 100, 85, 95, scale = 0.4)
...
visit_names <- c("Total", visit_names)
submissions <- c(n_total, submissions)

data.frame(visit_names, submissions) %>%
  kableExtra::kbl(booktabs = TRUE, linesep = "", col.names = c("Submission", "N")) %>% 
  kableExtra::kable_styling(latex_options = c("striped", "HOLD_position")) %>%
  kableExtra::row_spec(0, bold = TRUE) %>%
  kableExtra::row_spec(1, color = total_row_font, bold = TRUE)
```
...
baseline_data %>%
  ...
  kableExtra::kable_styling(latex_options = c("striped", "HOLD_position"), font_size = table_fs) %>%
  kableExtra::row_spec(0, bold = TRUE)
```
...
format_summary_table(all, age_statistics, facility_name) %>% 
  kableExtra::kbl(booktabs = TRUE, linesep = "", caption = age_cap) %>% 
  kableExtra::kable_styling(latex_options = c("striped", "HOLD_position"), font_size = table_fs) %>%
  kableExtra::row_spec(0, bold = TRUE) %>%
  kableExtra::row_spec(1, color = total_row_font, bold = TRUE)
```
...
if (Sys.getenv('TIMCI_COUNTRY') == 'Senegal') {
  sick_cap <- "Enfants malades"
} else {
  sick_cap <- "Sick children"
}
sickness_statistics <- list(
  "Facility" = list(
    "Sick children" = ~qwraps2::n_perc(sickness == 1, prec),
    "Other children" = ~qwraps2::n_perc(sickness == 0, prec)
    )
)
format_summary_table(all, sickness_statistics, facility_name) %>% 
  kableExtra::kbl(booktabs = TRUE, linesep = "", caption = sick_cap) %>% 
  kableExtra::kable_styling(latex_options = c("striped", "HOLD_position"), font_size = table_fs) %>%
  kableExtra::row_spec(0, bold = TRUE) %>%
  kableExtra::row_spec(1, color = total_row_font, bold = TRUE)
```
...
format_summary_table(all, sick_reason_statistics, facility_name) %>% 
  kableExtra::kbl(booktabs = TRUE, linesep = "", caption = consultion_sick_cap) %>% 
  kableExtra::kable_styling(latex_options = c("striped", "HOLD_position"), font_size = table_fs) %>%
  kableExtra::row_spec(0, bold = TRUE) %>%
  kableExtra::row_spec(1, color = total_row_font, bold = TRUE)
```
...
all <- nonsick
if (Sys.getenv('TIMCI_COUNTRY') == 'Senegal') {
  consultation_nonsick_cap <- "Principal motif de consultation pour les autres enfants."
} else {
  consultation_nonsick_cap <- "Primary reason for consultation for other children."
}
...
if (!is.null(all) & length(all) > 0 & nrow(all) > 0) {
  format_summary_table(all, reason_statistics, facility_name) %>% 
    kableExtra::kbl(booktabs = TRUE, linesep = "", caption = consultation_nonsick_cap) %>% 
    kableExtra::kable_styling(latex_options = c("striped", "HOLD_position"), font_size = table_fs) %>%
    kableExtra::row_spec(0, bold = TRUE) %>%
    kableExtra::row_spec(1, color = total_row_font, bold = TRUE)
}
...
all <- sick
if (Sys.getenv('TIMCI_COUNTRY') == 'Senegal') {
  cg_age_cap <- "Âge de l'accompagnant principal"
} else {
  cg_age_cap <- "Caregiver's age"
}
caregiver_statistics <- list(
  "Facility" = list(
    "Below 18" = ~qwraps2::n_perc(cg_eligibility == 0, prec),
    "18 and above" = ~qwraps2::n_perc(cg_eligibility == 1, prec)
    )
)
format_summary_table(all, caregiver_statistics, facility_name) %>% 
  kableExtra::kbl(booktabs = TRUE, linesep = "", caption = cg_age_cap) %>% 
  kableExtra::kable_styling(latex_options = c("striped", "HOLD_position"), font_size = table_fs) %>%
  kableExtra::row_spec(0, bold = TRUE) %>%
  kableExtra::row_spec(1, color = total_row_font, bold = TRUE)
```
...
```{r, results = "asis"}
if (Sys.getenv('TIMCI_COUNTRY') == 'Senegal') {
  u18_mother_cap <- "Proportion de mères adolescentes"
} else {
  u18_mother_cap <- "Proportion of underaged mothers"
}

all$underaged_mother[all$main_cg == 1 & all$cg_eligibility == 0] <- 1
all$underaged_mother[is.na(all$underaged_mother)] <- 0

mother_statistics <- list(
  "Mother's age" = list(
    "Below 18" = ~qwraps2::n_perc(underaged_mother == 1, prec),
    "18 and above" = ~qwraps2::n_perc(underaged_mother == 0, prec)
    )
)

format_summary_table(all, mother_statistics, facility_name) %>% 
  kableExtra::kbl(booktabs = TRUE, linesep = "", caption = u18_mother_cap) %>% 
  kableExtra::kable_styling(latex_options = c("striped", "HOLD_position"), font_size = table_fs) %>%
  kableExtra::row_spec(0, bold = TRUE) %>%
  kableExtra::row_spec(1, color = total_row_font, bold = TRUE)
```
...
format_summary_table(all, summary_statistics, facility_name) %>% 
  kableExtra::kbl(booktabs = TRUE, linesep = "", caption = multi_rec_cap) %>% 
  kableExtra::kable_styling(latex_options = c("striped", "HOLD_position"), font_size = table_fs) %>%
  kableExtra::row_spec(0, bold = TRUE) %>%
  kableExtra::row_spec(1, color = total_row_font, bold = TRUE)
```
...
out %>% 
  kableExtra::kbl(booktabs = TRUE, linesep = "", caption = consent_cap) %>% 
  kableExtra::kable_styling(latex_options = c("striped", "HOLD_position"), font_size = table_fs) %>%
  kableExtra::row_spec(0, bold = TRUE) %>%
  kableExtra::column_spec(2, color = ifelse(grepl(pattern = "(100.0%)", x = out[,1], fixed = TRUE), "red", "black")) %>%
  kableExtra::column_spec(3, color = ifelse(grepl(pattern = "(0.0%)", x = out[,2], fixed = TRUE), "red", "black")) %>%
  kableExtra::row_spec(1, color = total_row_font, bold = TRUE) 
```
...
if (Sys.getenv('TIMCI_COUNTRY') != 'India') {
  ...
  if (!is.null(pii)) {
    if (nrow(pii) > 0) {
      ...
      format_summary_table(all, phone_stats, facility_name) %>% 
        kableExtra::kbl(booktabs = TRUE, linesep = "") %>% 
        kableExtra::kable_styling(latex_options = c("striped", "HOLD_position"), font_size = table_fs) %>%
        kableExtra::row_spec(0, bold = TRUE) %>%
        kableExtra::row_spec(1, color = total_row_font, bold = TRUE)
...
if (Sys.getenv('TIMCI_COUNTRY') != 'India') {
  ...
  if (!is.null(pii)) {
    if (nrow(pii) > 0) {
      ...
      format_summary_table(all, contact_details_stats, facility_name) %>%
        kableExtra::kbl(booktabs = TRUE, linesep = "") %>%
        kableExtra::kable_styling(latex_options = c("striped", "HOLD_position"), font_size = table_fs) %>%
        kableExtra::row_spec(0, bold = TRUE) %>%
        kableExtra::row_spec(1, color = total_row_font, bold = TRUE)
...
if (nrow(baseline_data) > 0) {
  ...
  format_summary_table(all, clinical_presentation_statistics, facility_name) %>% 
    kableExtra::kbl(booktabs = TRUE, linesep = "") %>% 
    kableExtra::kable_styling(latex_options = c("striped", "HOLD_position"), font_size = table_fs) %>%
    kableExtra::row_spec(0, bold = TRUE) %>%
    kableExtra::row_spec(1, color = total_row_font, bold = TRUE)
...
if (nrow(baseline_data) > 0) {
  ...
  format_summary_table(all, pox_statistics, facility_name) %>% 
    kableExtra::kbl(booktabs = TRUE, linesep = "") %>% 
    kableExtra::kable_styling(latex_options = c("striped", "HOLD_position"), font_size = table_fs) %>%
    kableExtra::row_spec(0, bold = TRUE) %>%
    kableExtra::row_spec(1, color = total_row_font, bold = TRUE)
...
if (Sys.getenv('TIMCI_COUNTRY') == 'Senegal') {
  ...
  if (n_due_day7fu > 0) {
    ...
    cat(paste0("**", n_completed_day7fu, "** participants ont complété leur suivi à Jour 7."))
  }
} else {
  ...
  if (n_due_day7fu > 0) {
    ...
    cat(paste0("**", n_completed_day7fu, "** participants completed their Day 7 follow-up."))
  }
}
```
  ...
  if (nrow(day7fu_data) > 0) {
    ...
    day7_call_stats <- list(
      "District" = list(
        ...
        "Valid time window" = ~qwraps2::n_perc(cg_reached == 1 & (days >= day7_wmin) & (days <= day7_wmax), prec),
        "Lost to follow-up" = ~qwraps2::n_perc((cg_reached == 1 & (cg_ok == 0 | days > day7_wmax)), prec)
        )
    )
    
    format_summary_table(all, day7_call_stats, district) %>% 
      kableExtra::kbl(booktabs = TRUE, linesep = "") %>% 
      kableExtra::kable_styling(latex_options = c("striped", "HOLD_position"), font_size = table_fs) %>%
      kableExtra::row_spec(0, bold = TRUE) %>%
      kableExtra::row_spec(1, color = total_row_font, bold = TRUE)
...
if (!is.null(day7fu_data)) {
  if (nrow(day7fu_data) > 0) {
    ...
    day7_outcome_ss <- list(
      "District" = list(
        "Days" = ~qwraps2::mean_sd(days, denote_sd = "paren"),
        "Death" = ~qwraps2::n_perc(status_day7 == 3, prec),
        "Higher level visit" = ~qwraps2::n_perc(status_day7 == 2 | (status_day7 != 2 & hf_visit_day7 == 1 & hf_visit_type == 1), prec)
        )
      )
    format_summary_table(all, day7_outcome_ss, district) %>% 
      kableExtra::kbl(booktabs = TRUE, linesep = "") %>% 
      kableExtra::kable_styling(latex_options = c("striped", "HOLD_position"), font_size = table_fs) %>%
      kableExtra::row_spec(0, bold = TRUE) %>%
      kableExtra::row_spec(1, color = total_row_font, bold = TRUE) %>%
      kableExtra::footnote(general = "Mean and standard deviation (SD) are represented by the format *mean (SD)*")
  }
}
```
...
if (Sys.getenv('TIMCI_COUNTRY') == "Tanzania" || Sys.getenv('TIMCI_COUNTRY') == "India") {
  ...
  if (n_due_day28fu > 0) {
    ...
    cat(paste0("**", n_completed_day28fu, "** participants completed their Day 28 follow-up."))
  }
}
```
    ...
    if (nrow(raw_day28fu_data) > 0) {
      ...
      day28_call_ss <- list(
        "District" = list(
          ...
          "Lost to follow-up" = ~qwraps2::n_perc((cg_reached == 1 & (cg_ok == 0 | days > day28_wmax)), prec)
          )
      )
      format_summary_table(all, day28_call_ss, district) %>% 
        kableExtra::kbl(booktabs = TRUE, linesep = "") %>% 
        kableExtra::kable_styling(latex_options = c("striped", "HOLD_position"), font_size = table_fs) %>%
        kableExtra::row_spec(0, bold = TRUE) %>%
        kableExtra::row_spec(1, color = total_row_font, bold = TRUE)
