...
if (!is.null(wfa_data)) {
  if (length(wfa_data) > 0 & nrow(wfa_data) > 0) {
    ...
    ggplot2::ggplot(wfa_data, aes(x = week, y = u5_attendance_last_week)) +
      geom_bar(stat = "identity", fill = "#7dbbd6", width = 7) +
      ggplot2::scale_x_date(limits = c(as.Date(as.Date(start_date) - 7), as.Date(Sys.Date() + 1)),
                            breaks = "1 week",
                            date_labels = "%d.%m.%y") +
...
if (!is.null(day7fu_data)) {
  if (nrow(day7fu_data) > 0) {
    ...
    day7_outcome_ss <- list(
      "District" = list(
        "Time (mean, SD)" = ~qwraps2::mean_sd(days),
        "Cure known" = ~qwraps2::n_perc(cure_day7 == 1 | cure_day7 == 2 | cure_day7 == 3 | cure_day7 == 4, 2),
        "Higher level visit reported" = ~qwraps2::n_perc(hf_visit_day7 == 1 & hf_visit_type == 1, 2)
        )
      )
