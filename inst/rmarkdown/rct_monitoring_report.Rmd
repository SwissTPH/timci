...
```{r}
deidentified_facility_data <- study_data
```
  ...
  if (nrow(pii) > 0) {
    ...
    contact_details_stats <- list(
      "Contact details" = list(
        "Participant with 0 phone number" = ~qwraps2::n_perc(phone_nb == 0, 2),
        "Participant with 1 phone number" = ~qwraps2::n_perc(phone_nb == 1, 2),
        "Participant with 2 phone numbers" = ~qwraps2::n_perc((phone_nb == 1) & (phone_nb2 == 1), 2),
        "Participant with a community contact" = ~qwraps2::n_perc(cmty == 1, 2)
        )
    )
