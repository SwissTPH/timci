...
```{r, results='asis'}
if (Sys.getenv('TIMCI_COUNTRY') == 'Senegal') {
  cat(paste0("# Indicateurs clés de performance\n\n"))
  cat(paste0("## Taux de recrutement\n\n"))
} else {
  cat(paste0("# Key performance indicators\n\n"))
  cat(paste0("## Enrolment rate\n\n"))
}
...
timci::plot_enrolment_gauge(n_enrolled/(days_nb*3.8*9), "Enrolment versus global target", 100, 60, 85)
```
  ...
  if (nrow(day7fu_data) > 0) {
    ...
    day7_call_stats <- list(
      "Call outcome" = list(
        ...
        "Valid time window" = ~qwraps2::n_perc(cg_reached == 1 & (days >= wmin) & (days <= wmax), 2),
        "Lost to follow-up" = ~qwraps2::n_perc((cg_reached == 1 & (cg_ok == 0 | days > wmax)), 2)
        )
    )
