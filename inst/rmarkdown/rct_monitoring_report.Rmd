...
if (Sys.getenv('TIMCI_COUNTRY') == "Tanzania" || Sys.getenv('TIMCI_COUNTRY') == "India") {
  ...
  if (!is.null(raw_day28fu_data)) {
    if (nrow(raw_day28fu_data) > 1) {
      ...
      write(nrow(raw_day28fu_data), stderr())
      write(length(raw_day28fu_data), stderr())
...
plot9 <- timci::plot_gauge(n_completed_hospitfu/n_due_hospitfu, "Hospital follow-up rate", 100, 50, 80, scale = 0.4)
```
...
* [India, Tanzania] "the participant has completed the Day 7 and the Day 28 follow-ups visit"
* [Kenya, Senegal] "the participant has completed the Day 7 follow-ups visit"
...
selected_variables <- c("child_id", "facility_name", "date_visit", "district")
if (is_tanzania) {
  selected_variables <- c(selected_variables)#, "location_lvl1", "location_lvl2")
}
