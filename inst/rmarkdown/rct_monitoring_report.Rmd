  ...
  if (nrow(raw_hospit_data) > 0) {
    hospit_data <- format_hospital_data(raw_hospit_data) %>%
      dplyr::distinct_at(dplyr::vars(child_id),
                         .keep_all = TRUE)
    n_completed_hospitfu <- sum(hospit_data$found == 1)
  }
}

plot9 <- timci::plot_gauge(n_completed_hospitfu/n_due_hospitfu, "Hospital follow-up rate", 100, 85, 95, scale = 0.4)
```
...
if (!is.null(day7fu_data)) {
  if (nrow(day7fu_data) > 0) {
    all <- all %>%
      merge(y = succ_day7fu_data[, c("child_id", "proceed_day7", "hf_visit_type", "status_day7", "admission")],
            by = 'child_id',
            all.x = TRUE) %>%
...
all$hospitalised <- (all$status_day7 == 2) | (all$admission == 1)
...
completion_summary <- list(
  "Summary" = list(
    "Sought" = ~qwraps2::n_perc(found == 0 | found == 1, prec),
    "Completed" = ~qwraps2::n_perc(found == 1, prec),
    "Lost" = ~qwraps2::n_perc(found == 96, prec)
    )
)
...
hospitalisation_all <- all %>%
  dplyr::filter(hospitalised == 1)
...
descriptive_statistics <- list(
  "Summary" = list(
    "Date of birth known" = ~qwraps2::n_perc(dob_knwn == 1, prec),
...
all <- all %>%
  ...
  dplyr::mutate(missing_clinical_presentation = ifelse(danger_signs == 0 & sx_vomit == 0 & sx_less_feed == 0 & sx_cough == 0 & sx_difficulty_breath == 0 & sx_diarrhoea == 0 & sx_fever == 0 & sx_var == 96, 1, 0)) %>%
...
all <- all %>%
  dplyr::rowwise() %>%
  dplyr::mutate(cold = ifelse(2 %in% as.integer(unlist(strsplit(all$sx_var, split = ";"))), 1, 0))
all <- all %>%
  dplyr::rowwise() %>%
  dplyr::mutate(ear = ifelse(3 %in% as.integer(unlist(strsplit(all$sx_var, split = ";"))), 1, 0))
all <- all %>%
  dplyr::rowwise() %>%
  dplyr::mutate(eye = ifelse(4 %in% as.integer(unlist(strsplit(all$sx_var, split = ";"))), 1, 0))
all <- all %>%
  dplyr::rowwise() %>%
  dplyr::mutate(other_symptoms = ifelse(99 %in% as.integer(unlist(strsplit(all$sx_var, split = ";"))), 1, 0))
```
