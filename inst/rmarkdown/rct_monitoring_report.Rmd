...
if (!is.null(raw_day7fu_data)) {
  if (nrow(raw_day7fu_data) > 0) {
    ...
    n_completed_day7fu <- sum((as.Date(succ_day7fu_data$date_day0, "%Y-%m-%d") + day7_expandedwmax) <= Sys.Date(), na.rm = TRUE) 
    n_valid_day7fu <- sum(((as.Date(succ_day7fu_data$date_day0, "%Y-%m-%d") + day7_expandedwmax) <= Sys.Date()) & (succ_day7fu_data$days >= day7_wmin) & (succ_day7fu_data$days <= day7_wmax), na.rm = TRUE) 
  } else {
...
if (Sys.getenv('TIMCI_COUNTRY') == "Tanzania" || Sys.getenv('TIMCI_COUNTRY') == "India") {
  ...
  if (!is.null(raw_day28fu_data)) {
    if (nrow(raw_day28fu_data) > 0) {
      ...
      n_completed_day28fu <- sum((as.Date(succ_day28fu_data$date_day0, "%Y-%m-%d") + day28_expandedwmax) <= Sys.Date(), na.rm = TRUE) 
      n_valid_day28fu <- sum(((as.Date(succ_day28fu_data$date_day0, "%Y-%m-%d") + day28_expandedwmax) <= Sys.Date()) & (succ_day28fu_data$days >= day28_wmin) & (succ_day28fu_data$days <= day28_wmax), na.rm = TRUE) 
    } else {
...
data.frame(visit_names, submissions) %>%
  ...
  kableExtra::kable_styling(latex_options = c("striped", "HOLD_position")) %>%
  kableExtra::row_spec(0, bold = TRUE) %>%
  kableExtra::row_spec(1, color = "gray", bold = TRUE)
```
...
mother_statistics <- list(
  "Mother's age" = list(
    "Below 18" = ~qwraps2::n_perc(underaged_mother == 1 | (main_cg == 1 & cg_eligibility == 0), prec),
    "18 and above" = ~qwraps2::n_perc(underaged_mother == 0 | (main_cg == 1 & cg_eligibility == 1), prec)
    )
)
...
format_summary_table(all, consent_statistics, facility_name) %>% 
  kableExtra::kbl(booktabs = TRUE, linesep = "") %>% 
  kableExtra::kable_styling(latex_options = c("striped", "HOLD_position")) %>%
  kableExtra::row_spec(0, bold = TRUE)
```
...
if (!is.null(day7fu_data)) {
  if (nrow(day7fu_data) > 0) {
    ...
    day7_outcome_ss <- list(
      "District" = list(
        "Time (in s)" = ~qwraps2::mean_sd(days),
