...
if (Sys.getenv('TIMCI_IS_RCT') == 1) {
  ...
  n_all <- n_screened + n_day7fu + n_hospit + n_withdrawal
  visit_names <- c("Screening",
                 "Baseline visit",
                 "Repeat visit",
                 "Day 7 phone call",
                 ...
                 "Withdrawal",
                 "All")
  submissions <- c(n_screened, n_enrolled, n_repeat, n_day7fu, n_hospit, n_withdrawal, n_all)
}
...
if (nrow(facility_data) > 0) {
  plot(timci::generate_day_bar_plot(facility_data$date_visit, as.Date(Sys.Date() - w), Sys.Date()))
} else{
  cat('N/A\n\n')
}
```

### Baseline visit (enrolment)
```{r, echo=FALSE, warning=FALSE, results='asis'}
w <- 30
if (nrow(baseline_data) > 0) {
  plot(timci::generate_day_bar_plot(baseline_data$date_visit, as.Date(Sys.Date() - w), Sys.Date()))
} else{
  cat('N/A\n\n')
}
```
...
if (nrow(raw_day7fu_data) > 0) {
  plot(timci::generate_day_bar_plot(raw_day7fu_data$today, as.Date(Sys.Date() - w), Sys.Date()))
} else{
  cat('N/A\n\n')
}
```
...
if (nrow(raw_hospit_data) > 0) {
  plot(timci::generate_day_bar_plot(raw_hospit_data$today, as.Date(Sys.Date() - w), Sys.Date()))
} else{
  cat('N/A\n\n')
}
```
...
if (Sys.getenv('TIMCI_IS_RCT') == 1) {
  ...
  if (nrow(raw_day28fu_data) > 0) {
    plot(timci::generate_day_bar_plot(raw_day28fu_data$today, as.Date(Sys.Date() - w), Sys.Date()))
  } else{
    cat('N/A\n\n')
  }
}
...
df %>%
  ...
  geom_text(aes(label = lubridate::day(date_visit)), size = 2.5, color = "black") +
...
timci::generate_calendar_heatmap <- function(facility_data, date)
```
...
timci::generate_enrolment_hist(stats, facility, screened, 1, 15)
```

## Enrolment by facility
```{r, echo=FALSE, warning=FALSE}
timci::generate_enrolment_hist(stats, facility, children, 15, 20)
```
