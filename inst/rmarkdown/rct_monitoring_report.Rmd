...
if (nrow(facility_data) > 0) {
  ...
  if (Sys.getenv('TIMCI_COUNTRY') == 'Senegal') {
    cat(paste0("Ce rapport couvre la période du **", start_date, "** au **", end_date, "**."))
  } else {
    cat(paste0("This report covers the period from **", start_date, "** to **", end_date, "**."))
  }
}
```
...
if (!is.null(raw_day7fu_data)) {
  if (nrow(raw_day7fu_data) > 0) {
    plot(timci::generate_day_bar_plot(raw_day7fu_data$today, as.Date(Sys.Date() - w),
                                      as.Date(Sys.Date() + 1),
                                      submission_str))
  } else{
    cat('N/A\n\n')
...
if (!is.null(raw_hospit_data)) {
  if (nrow(raw_hospit_data) > 0) {
    plot(timci::generate_day_bar_plot(raw_hospit_data$today, as.Date(Sys.Date() - w),
                                      as.Date(Sys.Date() + 1),
                                      "Submissions"))
  } else{
