...
if (nrow(facility_data) > 0) {
  ...
  if (Sys.getenv('TIMCI_COUNTRY') != "Tanzania") {
    plot2 <- timci::plot_enrolment_gauge(n_enrolled/(days_nb*day_target), plot2_str, 100, 62.5, 100)
  } else{
    plot2 <- timci::plot_enrolment_gauge(n_enrolled/(days_nb*day_target), plot2_str, 100, 62.5, 100)
  }
}
...
```{r, results = "asis"}
timci::generate_time_hist_plot(facility_data)
```
