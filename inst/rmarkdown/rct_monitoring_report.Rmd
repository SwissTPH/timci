...
if (Sys.getenv('TIMCI_COUNTRY') == 'Senegal') {
  notice_str <- "\\begin{center}\\textbf{Pour usage interne uniquement. Ne pas partager en dehors des équipes recherche.}\\end{center}"
  ...
  notice_str <- "\\begin{center}\\textbf{For internal use only. Do not share outside research teams.}\\end{center}"
...
if (Sys.getenv('TIMCI_COUNTRY') == "Kenya") {
  ...
  enrolment_target <- (dispensary_enrolment_target * dispensary_nb + hc_enrolment_target * hc_nb) / facility_nb
} else{
  enrolment_target <- 8
}
if (Sys.getenv('TIMCI_COUNTRY') != "Tanzania") {
  ...
  cat(paste0("Daily enrolment target: ", dispensary_enrolment_target, " (dispensaries) and ", hc_enrolment_target, " (health centres)"))
}
```
...
if (nrow(facility_data) > 0) {
  ...
  if (Sys.getenv('TIMCI_COUNTRY') != "Tanzania") {
    plot2 <- timci::plot_enrolment_gauge(n_enrolled/(days_nb*enrolment_target*facility_nb), plot2_str, 100, 62.5, 100)
  } else{
    plot2 <- timci::plot_enrolment_gauge(n_enrolled/(days_nb*(dispensary_enrolment_target*dispensary_nb + hc_enrolment_target*hc_nb)), plot2_str, 100, 62.5, 100)
  }
}
```
...
```{r, results='asis'}
if (Sys.getenv('TIMCI_COUNTRY') == 'Senegal') {
  cat(paste0("### Taux de recrutement quotidien\n\n"))
} else {
  cat(paste0("### Daily enrolment rate\n\n"))
}
if (nrow(baseline_data) > 0) {
  timci::generate_day_bar_plot(facility_data$date_visit, as.Date(Sys.Date() - w),
                               as.Date(Sys.Date() + 1),
                               ylbl = submission_str,
                               ylim = c(0, 100),
                               rref = enrolment_target * facility_nb)
}
```
...
age_statistics <- list(
  "Facility" = list(
    ...
    "0 - 2 months" = ~qwraps2::n_perc(age_incl == 1 & yg_infant == "0-2m", pres),
...
summary_statistics <- list(
  "Facility" = list(
    "Below 18 years old" = ~qwraps2::n_perc(cg_eligibility == 0, pres),
    "Above 18 years old" = ~qwraps2::n_perc(cg_eligibility == 1, pres)
    )
)
...
mother_statistics <- list(
  "Mother's age" = list(
    ...
    "Above 18" = ~qwraps2::n_perc(underaged_mother == 0 | (main_cg == 1 & cg_eligibility == 1), pres)
    )
)
...
for (i in 1:nrow(facilities)) {
  ...
  if (!is.null(facility_data_tmp) & length(facility_data_tmp) > 0 & nrow(facility_data_tmp) > 0) {
    facility_data_tmp$timestamp <- strptime(facility_data_tmp$start, format = "%H:%M:%S")
    facility_data_tmp$hours <- as.numeric(format(facility_data_tmp$timestamp, format = "%H"))
    hist(facility_data_tmp$hours,
         main = paste0("Screening times for ", fname), 
         xlab = "Times", 
         border = "black", 
         col = "blue",
         las = 1, 
