...
facility_data <- merge(facility_data, params$research_facilities[, c("deviceid", "facility_name")], by.x = 'device_id', by.y = 'deviceid')
```
...
timci::generate_enrolment_hist(stats, device_id, screened, min_target_day+1, max_target_day, "Daily average of children screened", "facilities")
```
...
timci::generate_enrolment_hist(stats, device_id, children, min_target_day, max_target_day, "Daily average of children enrolled", "facilities")
```
...
age_statistics <- list(
  "Facility" = list(
    "Above 59 months" = ~qwraps2::n_perc(age_incl == 0, 2),
    "2 - 59 months" = ~qwraps2::n_perc(age_incl == 1 & yg_infant == "2-59m", 2),
    "0 - 2 months" = ~qwraps2::n_perc(age_incl == 1 & yg_infant == "0-2m", 2),
...
sickness_statistics <- list(
  "Facility" = list(
    "Sick children" = ~qwraps2::n_perc(sickness == 1, 2),
    "Other children" = ~qwraps2::n_perc(sickness == 0, 2)
    )
)
...
sick_reason_statistics <- list(
  "Facility" = list(
    ...
    "Trauma" = ~qwraps2::n_perc(consult_reason == 2 & sickness == 1, 2),
    "Immunisation" = ~qwraps2::n_perc(consult_reason == 3 & sickness == 1, 2),
    "Monitoring" = ~qwraps2::n_perc(consult_reason == 4 & sickness == 1, 2)
    )
)
...
summary_statistics <- list(
  "Facility" = list(
    "First" = ~qwraps2::n_perc(prev_enrl == 3 | prev_enrl == 98, 2),
    "Previous" = ~qwraps2::n_perc(prev_enrl == 1 | prev_enrl == 2, 2),
    "Last <= 28 days" = ~qwraps2::n_perc(repeat_consult == 1, 2),
    "Last > 28 days" = ~qwraps2::n_perc((prev_enrl == 1 | prev_enrl == 2) & repeat_consult == 0, 2)
    )
)
st <- qwraps2::summary_table(all, summary_statistics)
st1 <- qwraps2::summary_table(dplyr::group_by(all, facility_name), summary_statistics)
