...
if (!is.null(day7fu_data)) {
  if (nrow(day7fu_data) > 0) {
    ...
    day7_call_stats <- list(
      "District" = list(
        "Successful call" = ~qwraps2::n_perc(call_ok == 1, prec, na_rm = TRUE),
  ...
  if (nrow(day7fu_data) > 0) {
    ...
    day7_outcome_ss <- list(
      "summary" = list(
        ...
        "Higher level visit" = ~qwraps2::n_perc(status_day7 == 2 | (status_day7 != 2 & hf_visit_day7 == 1 & hf_visit_type == hospit_value), prec, na_rm = TRUE)
        )
      )
    out <- format_summary_table(all, day7_outcome_ss, facility_name) 
    out %>% 
