...
cat("\\newpage")
if (Sys.getenv('TIMCI_COUNTRY') == 'Senegal') {
  title2_str1 <- "## Nombre de soumissions au cours des 30 derniers jours\n\n"
  title2_str2 <- "## Recrutement hebdomadaire\n\n"
} else {
  title2_str1 <- "## Number of submissions during the last 30 days\n\n"
  title2_str2 <- "## Weekly enrolment\n\n"
}
```
...
if (nrow(spa_sco_data) > 0) {
  plot(timci::generate_day_bar_plot(spa_sco_data$date,
                                    as.Date(Sys.Date() - w),
                                    as.Date(Sys.Date() + 1),
                                    ylbl = submission_str))
}
...
```{r, results = "asis"}
cat(title2_str2)
if (!is.null(spa_sco_data) & length(spa_sco_data) > 0 & nrow(spa_sco_data) > 0) {
  p <- timci::generate_week_bar_plot(date_vec = spa_sco_data$date,
