...
```{r, eval = cond, results = "asis", eval=(is_kenya|is_senegal)}
obs_count <- all %>%
  dplyr::filter(Q1 == 1) %>%
...
obs_count %>% 
  kableExtra::kbl(booktabs = TRUE,
                  ...
                  linesep = "",
                  align = c("c", "c", "c"),
                  caption = "Number of consultation observations by providers in Q2") %>% 
  kableExtra::kable_styling(latex_options = c("HOLD_position", "repeat_header"), font_size = table_fs) %>%
  kableExtra::row_spec(0, bold = TRUE) %>%
  kableExtra::collapse_rows(columns = 1, valign = "top") 
```

```{r, eval = cond, results = "asis", eval=is_india}
obs_count <- all %>%
  dplyr::filter(Q1 == 1) %>%
  dplyr::group_by(facility_name, hcp_id) %>%
  dplyr::summarise("Observed children" = n_distinct(child_id))
...
```{r, eval = cond, results = "asis", eval=is_tanzania}
d1 <- "Kaliua"
obs_count <- all %>%
