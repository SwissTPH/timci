...
```{r, eval=is_ls}
all <- all %>% 
  dplyr::mutate(Q1 = as.Date(date) <= as.Date(Sys.getenv("TIMCI_SPA_Q1_END_DATE")))
```

```{r, eval=is_rct}
all <- all %>% 
  dplyr::mutate(quarter = ifelse(as.Date(date) <= as.Date(Sys.getenv("TIMCI_SPA_Q2_END_DATE")),
                                 "Q2",
                                 "Q3"))
```
...
obs_count %>% 
  kableExtra::kbl(booktabs = TRUE,
                  ...
                  linesep = "",
                  align = c("c", "c", "c"),
                  caption = "Number of consultation observations by providers in Q1") %>% 
  kableExtra::kable_styling(latex_options = c("HOLD_position", "repeat_header"), font_size = table_fs) %>%
  kableExtra::row_spec(0, bold = TRUE) %>%
...
```{r, results="asis", eval=cond & is_ls, fig.cap=heatmap2_cap, fig.height=figsize}
all_q1 <- all %>% dplyr::filter(Q1 == 1)
if (!is.null(all_q1)) {
